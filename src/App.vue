<template>
  ...
  <hr />
  <h2 v-for="(name, index) in names" :key="index">{{ index+1 }}. {{ name }}</h2>
  
  <table border="1" cellspacing="0" cellpadding="5" :style="{display: 'inline-block'}">
    <tr>
      <th>#</th>
      <th>Name</th>
      <th>NIM</th>
  ...
  <h1>Count : {{ count }} <span>{{badge}}</span></h1> 
  <button @click="changeCount($event), changeBadge('Changed Up!')" num=1>Increment 1</button>
  <button @click="changeCount($event), changeBadge('Changed Up!')" num="5">Increment 5</button>
  <button @click="changeCount($event), changeBadge('Changed Down!')" num="-1">Decrement 1</button>
  <button @click="changeCount($event), changeBadge('Changed Down!')" num="-5">Decrement 5</button>
  ...
  <div class="container col-4" v-once>
    ...
    Jumlah : {{ count }}
    <button class="btn btn-primary" @click="changeCount" num="1">Tambah 1</button>
    <br>
    <p v-pre>
      {{ content }}
    </p>
